...
type Gamma = { board: Sigma; routes: string[]; timers: Set<any>; };
type EffectContext = { gamma: Gamma; phi: (env: Gamma) => Gamma; };
type ReversibleEffect = { forward: (env: Gamma) => Gamma; backward: (env: Gamma) => Gamma; };

// ═══════════════════════════════════════════════════
// 2. 核心数学函数 (复用)
// ═══════════════════════════════════════════════════
function track(effect: ReversibleEffect, ctx: EffectContext): EffectContext {
  return { gamma: effect.forward(ctx.gamma), phi: (env) => ctx.phi(effect.backward(env)) };
}
function recover(ctx: EffectContext): EffectContext {
  return { gamma: ctx.phi(ctx.gamma), phi: (env) => env };
}
...
// ═══════════════════════════════════════════════════
// 3. 框架核心 (The Framework)
// ═══════════════════════════════════════════════════
interface PluginDef { name: string; inject: string[]; apply: (api: ContextProxy) => void; }
interface ContextProxy { 
  // 插件委托框架，把自己提供的服务登记到全局白板 board 上，并把这次写入的可撤销操作记在自己的账本 ctx 上。
  provide: (k: string, v: any) => void; 
  // 插件向框架/白板请求获取某个服务。
  get: (k: string) => any;
}
interface PluginInstance { def: PluginDef; deps: Set<string>; active: boolean; ctx: EffectContext; }
...
class MiniFramework {
  ...
  constructor() {
    this.globalCtx = { gamma: { board: new Map(), routes: [], timers: new Set() }, phi: (env) => env };
  }
  ...
  use(def: PluginDef) {
    this.plugins.set(def.name, {
      ...
      active: false,
      ctx: { gamma: this.globalCtx.gamma, phi: (env) => env } // 每个插件有自己的账本
    });
  }

  // 【框架级核心】：真正修改 Sigma (白板) 的动作！
  // 对应论文的 set(k, v) : E*_Σ
  // 注意：它不属于插件，它属于框架。它接收 providerCtx，是为了把“撤销小票”记在提供者的账上！
  private executeSetEffect(k: string, v: any, providerCtx: EffectContext): EffectContext {
    const setEffect: ReversibleEffect = {
      forward: (env) => ({ ...env, board: new Map(env.board).set(k, v) }),
      backward: (env) => {
        const b = new Map(env.board); b.delete(k);
        return { ...env, board: b };
      }
    };
    return track(setEffect, providerCtx);
  }

  // 【插件级代理】：框架给插件发的“遥控器”
  // 解答你的疑惑：为什么 ContextProxy 里有 set/provide？
  // 因为插件不能直接改 Sigma，它必须“委托”框架去改，并附带自己的账本(providerCtx)。
  private createContextProxy(pluginName: string): ContextProxy {
    return {
      provide: (k: string, v: any) => {
        const p = this.plugins.get(pluginName)!;
        const oldBoard = p.ctx.gamma.board;
        
        // 1. 委托框架执行 set 动作，并把小票记在该插件的 ctx 上
        p.ctx = this.executeSetEffect(k, v, p.ctx);
      ...
      get: (k: string) => this.globalCtx.gamma.board.get(k)
    };
  }

  // 检查所有插件，是否满足 active 或 deactive
  private checkAll(oldBoard: Sigma, newBoard: Sigma) {
    for (const [name, p] of this.plugins) {
      ...
      if (state === 'activating' && !p.active) {
        p.active = true;
        console.log(`  ➡️ [activating] ${name} 依赖齐了，开工`);
        p.def.apply(this.createContextProxy(name)); // 传入遥控器
      } else if (state === 'deactivating' && p.active) {
        p.active = false;
        console.log(`  🧹 [deactivating] ${name} 依赖没了，停工并撤销`);
        p.ctx = recover(p.ctx); // 一键回滚该插件的所有改动（包括它提供的服务！）
        this.globalCtx.gamma = p.ctx.gamma;
      }
    }
  }
...
app.use({
  name: 'db-driver', inject: [],
  apply: (ctx_proxy) => {
    console.log('    [db-driver] 连接数据库...');
    // 插件按下遥控器上的 provide 按钮
    ctx_proxy.provide('database', { query: () => ['user1'] });
  }
});

app.use({
  name: 'user-api', inject: ['database'],
  apply: (ctx_proxy) => { console.log('    [user-api] 注册 /users 路由'); }
});

console.log('--- 启动系统 ---');
// 为了绕过漏洞打的补丁，实际不应该手工设 active 的，
// 也不应该手工调用 PluginDef 的 apply 的。 
app.plugins.get('db-driver')!.active = true;
app.plugins.get('db-driver')!.def.apply(app.createContextProxy('db-driver'));
