...
可以把第 4 节从“概念状态机”扩写成“可运行的工程状态机”。下面给出一版改写后的第 4 节，尽量复用前文已经定义过的 `Gamma`、`Sigma`、`ReversibleEffect`、`UnifiedContext`，并用 TypeScript 说明每个状态迁移到底怎么实现。
...
class Scheduler implements SchedulerLike {
  ...
  async use(plugin: Plugin): Promise<Fiber> {
  ...
  private depsSatisfied(plugin: Plugin): boolean {
