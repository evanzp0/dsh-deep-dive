...
#### 1.2.4 第四个公式：effect context —— 系统的"存档槽"与"时光倒流说明书"
...
**符号拆解：**
*   $\partial\Gamma$（读作 "partial Gamma"）：论文发明的一种新类型，叫 effect context，表示带有副作用的上下文。
...
*   $\gamma$：**当前进度**（当前环境状态）。
*   $\varphi$（读作 phi）：**一键读档指令**（撤销累积器）。它的初值是"什么都不做"（$\mathrm{id}$，即 `x => x`）。因为在你还没做任何改动时，如果要求你"撤销"，那当然就是"保持原样，什么都不用干"。

**用 TypeScript 代码"翻译"这个公式：**
